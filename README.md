...
- **重入按句柄，不按 goroutine。** Go 里没有稳定的线程／goroutine 身份，所以按库句柄
  算。与[事务只认句柄](#事务只认句柄不认调用它的那段代码)出自同一个原因。
...
原文件改名留作 `<库名>-backup.db`；搬不干净的话，跳过了哪些页写在新库的
...
| `$query` | 内层 SQL 的一行结果 | 拿一条查询的结果当另一条的数据源 |
...
- **一次提交在写盘中途失败后，这个库实例作废**（`ErrBroken`），要关掉重开。那时内存里
  的文件头已经带上了这次提交的一部分改动，而它们没有对应的日志页可供恢复丢弃；继续用
  下去，下一个事务提交会把这半个状态一并写进文件。重开会从文件读回干净的那份。
...
- **`Close` 之后一切操作返回 `ErrClosed`**，`Close` 本身幂等。只读打开的库关闭时
  不做检查点——它一个字节都不写。
